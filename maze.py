...
class Maze:
   ...
   # cell logic for this would be x1, x1 + cell_sizex1, ect w y
    ...
    def _solve_r(self, i, j):
        ...
        #right
        if i < self._num_cols -1 and (self._cells[i+1][j].has_left_wall == False) and self._cells[i+1][j].visited == False:
            self._cells[i][j].draw_move(self._cells[i+1][j])
            if self._solve_r(i+1,j):
                return True
            self._cells[i+1][j].draw_move(self._cells[i][j], True)
        ...
        if j < self._num_rows - 1 and (self._cells[i][j+1].has_top_wall == False) and self._cells[i][j+1].visited == False:
            self._cells[i][j].draw_move(self._cells[i][j+1])
            if self._solve_r(i,j+1):
                return True
            self._cells[i][j+1].draw_move(self._cells[i][j], True)
